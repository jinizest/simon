...
'''
 ...
 : forked from script written by 날자, 룰루해피, 따분, Susu Daddy, clipman, vifrost
...
#       heatmode_dic = {'heat': '11', 'off': '01'}
 ...
 # 2023.08 AC 추가 #24.04.29 simon도 추가~ ㅎㅎ -> 240510 simon 수정
    ...
    # 'myhome' 대신 'livingroom' 사용 @241114
    elif 'elevator' in topic_d:
        dev_id = device_h_dic['elevator'] + room_h_dic['livingroom']  # 'myhome' 대신 'livingroom' 사용
        ...
        if command == 'on':
            ...
            if ret_elevator == False:
                ...
                return
            threading.Thread(target=mqttc.publish, args=("kocom/livingroom/elevator/state", state_on)).start()
            if config.get('Elevator', 'rs485_floor', fallback=None) == None:
                threading.Timer(5, mqttc.publish, args=("kocom/livingroom/elevator/state", state_off)).start()
        elif command == 'off':
            threading.Thread(target=mqttc.publish, args=("kocom/livingroom/elevator/state", state_off)).start()
...
def packet_processor(p):
    ...
    if p['type'] == 'ack' and p['src'] == 'wallpad':  # ack from wallpad
        ...
        #     mqttc.publish('kocom/room/ac/' + p['dest_subid'] + '/state', json.dumps(state))
            ...
            logtxt='[MQTT publish|light] room[{}] data[{}]'.format(p['dest_subid'], state)
            mqttc.publish("kocom/{}/light/state".format(p['dest_subid']), json.dumps(state))
        ...
        mqttc.publish("kocom/livingroom/elevator/state", json.dumps(state))
...
def publish_discovery(dev, sub=''):
    if dev == 'fan':
        ...
        payload = {
            ...
            'cmd_t': "kocom/livingroom/elevator/command",
            'stat_t': "kocom/livingroom/elevator/state",
            'val_tpl': "{{ value_json.state }}",
            'pl_on': 'on',
            'pl_off': 'off',
            'ic': 'mdi:elevator',
...
#               'uniq_id': '{}_{}_{}{}'.format('kocom', 'wallpad', dev, num),      # 20221108 주석처리
            ...
            logtxt='[MQTT Discovery|{}{}{}] data[{}]'.format(sub, dev, num, topic)
            mqttc.publish(topic, json.dumps(payload))
            if logtxt != "" and config.get('Log', 'show_mqtt_publish') == 'True':
                logging.info(logtxt)
